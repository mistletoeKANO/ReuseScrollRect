...
namespace ReuseScrollRect.Runtime
{
    ...
    public abstract class RScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        //==========LoopScrollRect==========
        /// <summary>
        ...
        [HideInInspector]
        [NonSerialized]
        public IItemFactory itemFactory = null;
        ...
        [HideInInspector]
        [NonSerialized]
        public IItemSize sizeHelper = null;
        ...
        /// </summary>
        protected int startItemIndex = 0;
        ...
        protected int StartLine
        {
            get
            {
                return Mathf.CeilToInt((float)(startItemIndex) / contentConstraintCount);
            }
        }
        ...
        protected int CurrentLines
        {
            get
            {
                return Mathf.CeilToInt((float)(lastItemIndex - startItemIndex) / contentConstraintCount);
            }
        }
        ...
        public int GetFirstItem(out float offset)
        {
            ...
            if (lastItemIndex > startItemIndex)
            {
                ...
                while (size + offset <= 0 && startItemIndex + idx + contentConstraintCount < lastItemIndex)
                {
                    offset += size;
        ...
        public int GetLastItem(out float offset)
        {
            ...
            if (lastItemIndex > startItemIndex)
            {
                ...
                while (size + offset <= 0 && startItemIndex < lastItemIndex - idx - contentConstraintCount)
                {
                    offset += size;
                    idx += contentConstraintCount;
            ...
            while (needMoving)
            {
                ...
                if (!m_Dragging)
                {
                    ...
                    else
                    {
                        ...
                        if (totalCount >= 0)
                        {
                            if (offset > 0 && lastItemIndex == totalCount && !reverseDirection)
                            {
                            ...
                            else if (offset < 0 && startItemIndex == 0 && reverseDirection)
                            {
        ...
        protected abstract void ItemRefreshEvent(Transform transform, int index);
        ...
        protected void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < m_Content.childCount; i++)
                {
                    ...
                    {
                        ItemRefreshEvent(m_Content.GetChild(i), lastItemIndex);
                        lastItemIndex++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int endItem = 0, bool alignStart = false)
        {
            ...
            lastItemIndex = reverseDirection ? endItem : totalCount - endItem;
            startItemIndex = lastItemIndex;

            if (totalCount >= 0 && startItemIndex % contentConstraintCount != 0)
            {
                startItemIndex = (startItemIndex / contentConstraintCount) * contentConstraintCount;
            }
        ...
        protected void RefillCells(int startItem = 0, bool fillViewRect = false, float contentOffset = 0)
        {
            if (!Application.isPlaying)
                return;

            startItemIndex = reverseDirection ? totalCount - startItem : startItem;
            if (totalCount >= 0 && startItemIndex % contentConstraintCount != 0)
            {
                startItemIndex = (startItemIndex / contentConstraintCount) * contentConstraintCount;
            }
        ...
        protected float NewItemAtStart(bool includeSpacing = true)
        {
            ...
            {
                startItemIndex--;
                RectTransform newItem = GetFromTempPool(startItemIndex);
        ...
        protected float DeleteItemAtStart()
        {
            ...
            if ((m_Dragging || m_Velocity != Vector2.zero) && totalCount >= 0 && lastItemIndex >= totalCount - contentConstraintCount)
            {
                return 0;
            }
        ...
        {
            if (totalCount >= 0 && lastItemIndex >= totalCount)
            {
                return -1;
            }
            float size = 0;
            ...
            {
                RectTransform newItem = GetFromTempPool(lastItemIndex);
        ...
        {
            if ((m_Dragging || m_Velocity != Vector2.zero) && totalCount >= 0 && startItemIndex < contentConstraintCount)
            {
                return 0;
            }
            int availableChilds = m_Content.childCount - deletedItemTypeStart - deletedItemTypeEnd;
            ...
            for (int i = 0; i < contentConstraintCount; i++)
            {
                ...
                availableChilds--;
                lastItemIndex--;
                if (lastItemIndex % contentConstraintCount == 0 || availableChilds == 0)
                {
        ...
        void UpdateCachedData()
        {
            ...
            bool hScrollbarIsChild = (!m_HorizontalScrollbarRect || m_HorizontalScrollbarRect.parent == trans);
            bool vScrollbarIsChild = (!m_VerticalScrollbarRect || m_VerticalScrollbarRect.parent == trans);
        ...
        //==========LoopScrollRect==========
        private void GetHorizontalOffsetAndSize(out float totalSize, out float offset)
        {
            if (sizeHelper != null)
            {
        ...
        public float horizontalNormalizedPosition
        {
            get
            {
                UpdateBounds();
                //==========LoopScrollRect==========
                if (totalCount > 0 && lastItemIndex > startItemIndex)
                {
        ...
        private Bounds GetBounds4Item(int index)
        {
            ...
            int offset = index - startItemIndex;
        ...
        private Vector2 CalculateOffset(Vector2 delta)
        {
            return InternalCalculateOffset(ref m_ViewBounds, ref m_ContentBounds, m_Horizontal, m_Vertical, m_MovementType, ref delta,
                GetDimension(delta), startItemIndex, lastItemIndex, totalCount);
        }
